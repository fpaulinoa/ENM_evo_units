...
# Results New/<unit>/. Caiman_latirostris (whole species) is excluded
# excluded from this script by design; it belongs to Article 1, not Article 2.)
...
# ---------------------------------------------------------------------------
# Dynamically list the evolutionary units/aggregates to process, excluding
# Caiman_latirostris (whole species, stays only in Article 1's MaxEnt)
# ---------------------------------------------------------------------------
...
cat("Unidades a processar no RF:", length(species_list), "\n")
print(species_list)
...
for (species in species_list) {
  ...
  tryCatch({
    ...
    cat("\n========== Iniciando RF para", species, "==========\n")

    # ---- 1. Load the already-prepared SWD data ----
    ...
    # ---- 5. Loop: 20 runs x 5-fold cross-validation ----
    for (run in 1:n.runs) {
      ...
      write.csv(importance_df, paste0(eval.dir, "rf_importance_progresso_", species, ".csv"), row.names = FALSE)
      write.csv(curves_df, paste0(eval.dir, "rf_curves_progresso_", species, ".csv"), row.names = FALSE)

      cat("RF concluído -", species, "- run", run, "de", n.runs, "\n")
    }

    # ---- 6. Save evaluations and filter valid replicates ----
    ...
    if (nrow(valid_replicates) > 0) {
      ...
      message("Nenhuma réplica válida (RF) encontrada para ", species, ".")
    }
    ...
    if (nrow(importance_df) > 0) {
      ...
      cat("Importância e curvas de resposta (RF) salvas para", species, "\n")
    } else {
      message("Nenhuma réplica válida suficiente para importância/curvas de resposta (RF) - ", species)
    }

  }, error = function(e) {
    message("Falhou o processamento RF de ", species, ": ", conditionMessage(e))
  })
}

cat("\nProcessamento RF completo para as unidades evolutivas/agregados (excluindo Caiman_latirostris).\n")
